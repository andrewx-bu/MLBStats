...
class Fetcher {
    ...
    func fetchPredictionStats<T: Decodable>(statType: StatType) async throws -> [T] {
        let urlString = "https://www.fangraphs.com/api/leaders/major-league/data?pos=all&stats=\(statType.rawValue)&lg=all&qual=y&pageitems=9999&rost=1&season=2024&month=11"
        guard let url = URL(string: urlString) else {
            print("fetchStats (\(statType.rawValue)): Invalid URL string: \(urlString)")
            throw URLError(.badURL)
        }
